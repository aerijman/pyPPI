from pdbReader import PDBReader, angle
...
MIN_LENGTH_HBOND=1.97 #wikipedia: The typical length of a hydrogen bond in water is 197 pm
...
class hbonds(object):
    """
        class for calculation and find of hbonds and hbonds with water on interface
        both intra and inter are found
    """
        ...
        for atom in [a for a in interfacesAAA if a.atomType!='H']:
            for water in self.pdb.waters:
                ...
                if sqDist<WATER_LENGTH:
                    ...
                    elif atom.pseudoChain=='B' and (water in probablInterface) and min(minDistanceA[water][1], sqDist)<3.5:
    ...
    def hbonds(self, interfaces):
        ...
        """
        #foreach interface atom from A
        logging.debug('start hbonds for interface atoms')
        i= time.clock()        
        donorsAcceptors=set()
        for atom in [a for a in interfaces]:
            # dont ignore by chain - both inter and intra
            for atom2 in [a for a in self.pdb.ktree.findByDistance(query_point=atom.coord, distance=MAX_HBOND) if a!=atom]:
        ...
        logging.debug('end hbonds (%s secs)', (time.clock()-i))
        ...
        for donor,donoarOrbital,hAtom,testD in DonAcceptor.donors(pDonor, self.pdb.getNextAtoms):
            for acceptor,acceptorOrbital,preAtom,testA in DonAcceptor.acceptors(pAcceptor, self.pdb.getNextAtoms):
                ...
                if dist<MAX_DA and HA<MAX_HA:
                    ...
                    phi=angle(preAtomAcceptor, sqrt(preAtom.distance(hAtom)),sqrt(acceptor.distance(hAtom)),radians=True)
                    d_a_aa=angle(dist, preAtomDonor,preAtomAcceptor)
                    #for dbeug
                    #return (True, 0)
    ...
    def checkHbond(self, pDonor, pAcceptor):
        ...
        for donor,donoarOrbital,hAtom,testD in DonAcceptor.donors(pDonor, self.pdb.getNextAtoms):            
            for acceptor,acceptorOrbital,preAtom,testA in DonAcceptor.acceptors(pAcceptor, self.pdb.getNextAtoms):
                ...
                dist=sqrt(pAcceptor.distance(donor))
                if dist<MIN_LENGTH_HBOND:
                    logging.info('WARNING no hbond between', donor, pAcceptor, '. distance is too small: {0:.3f}'.format(dist))
                    return False,0
                theta=angle(DH,dist,HA,radians=True)
                phi=angle(sqrt(preAtom.distance(acceptor)), sqrt(preAtom.distance(hAtom)),sqrt(acceptor.distance(hAtom)),radians=True)
                logging.info('Orbitals Donor: %s Acceptor: %s . Theta %s Phi %s'%(donoarOrbital,acceptorOrbital,theta,phi))
                f=self.fAngleDep(donoarOrbital,acceptorOrbital,theta,phi)
                energy=self.leenardJones(dist)*f
                if DEBUG:
                    #if donor.resId in [63,23]:
                    print '%s%s%s-%s%s%s E=%s Dist=%s Phi=%s Theta=%s '%(donor.chain,donor.resId,donor.symbol,acceptor.chain,acceptor.resId,acceptor.symbol,energy,dist,phi,theta)
                if energy<HBOND_ENERGY:
                    if haveHbond:
                       eHB=min(eHB,energy)
                    else:
                        eHB=energy
                    # check for our change of 90 deg ang needed)
                    #if testA['orbital']=='sp3' and testD['orbital']=='sp2':
                    #    print 'FOUND between', donor, pAcceptor, hAtom, '(',hAtom.symbol,')', '. theta {0:.3f}'.format(theta*180/math.pi), '. phi {0:.3f}'.format(phi*180/math.pi)
    ...
    def hbondsOutput(self,donorsAcceptors):
        import string
        hBondsoutput=self.pdb.getFile('.hbonds.txt')
        print>>hBondsoutput,("			 "+self.pdb.name+"\n")
        chainDonors=dict()
        chainAcceptors=dict()
        for donor,acceptor,eHb in donorsAcceptors:
            ...
            print>>hBondsoutput,'{0:<6}{1:<3}{2:<3} (Donor) <--> {3:<6} {4:<3}  {5:<3} (Acceptor)   Energy = {6}'.format(
                donor.symbol,donor.chain, donor.resId,acceptor.symbol,acceptor.chain, acceptor.resId,eHb)

        hBondsoutput.close()
        #TODO: if atom is acceptor to many
        pmlHbonds=self.pdb.getFile('.hbonds.pml')
        donorSelection,accSelection=[],[]
        for chain,donorsList in chainDonors.iteritems():
            donorSelection.append(' res '+string.join(donorsList,'+')+' and chain '+chain)
        for chain,acceptorList in chainAcceptors.iteritems():
            accSelection.append(' res '+string.join(acceptorList,'+')+' and chain '+chain)
        
        print>>pmlHbonds,'sel '+string.join(donorSelection,' or ')
        print>>pmlHbonds,'show lines,sele'
        print>>pmlHbonds,'set_name sele,Donor'
        print>>pmlHbonds,'sel '+string.join(accSelection,' or  ')
        print>>pmlHbonds,'show lines,sele'
        print>>pmlHbonds,'set_name sele,Acceptor'

    def H2Oprintout(self, hbonds, minDistanceA, minDistanceB, interfaceB):
            #h2o interface
            interfaceFile=self.pdb.getFile('.interfaces.txt')
            print>>interfaceFile,("			 "+self.pdb.name+"\n")
            
            for water in self.pdb.waters:
                if water in interfaceB:
                    atomA,neighborDis=minDistanceA[water]
                    atomB,neighborDis=minDistanceB[water]
                    strLine="{0:^3} {1}   -   {2:^4} {3:^3} {4}  {5:^4}  & {6:^4} {7:^3} {8}   {9:^4}".format(water.resId,water.chain,
                                   atomA.symbol, atomA.residue, atomA.chain, atomA.resId,
                                   atomB.symbol,atomB.residue,atomB.chain, atomB.resId)
                    print>>interfaceFile,strLine
            interfaceFile.close()

            #hbonds file
            hbondsFile=self.pdb.getFile('.h2oHbonds.txt')
            print>>hbondsFile,("			 "+self.pdb.name)
            print>>hbondsFile,("	      ("+str(len(hbonds))+" Hbonds) \n")
            for atom,water in hbonds:
                strLine="HOH  {0:^2} {1:^2}   -   {2:^3} {3:^4} {4:^2} {5:^4}".format(water.chain,water.resId,atom.symbol,atom.residue,atom.chain,atom.resId)
                print>>hbondsFile,strLine
            hbondsFile.close()
            print 'found ',len(hbonds),' hbonds with water on interface'

def main():
    from ASA import ASA
    logging.basicConfig(level=logging.INFO)
    #a=PDBReader.readFile(u"./pdbs/1AK4_FH.pdb",interfaceParts=['A','D'])
    #bonds=hbonds(a)
    #bonds.execute()
    global DEBUG
    DEBUG=True
    a=PDBReader.readFile(u"./pdbs/1UUG_FH.pdb",interfaceParts=['B','A'])
    #a=PDBReader.readFile(u"./pdbs/1DQJ_FH.pdb",interfaceParts=['AB','C'])
    bonds=hbonds(a)
    ...
    interface.execute()
    interfaceList=[]
    for atom,diffASA in interface.diffASAperAtom.iteritems():
        if diffASA!=0:
            interfaceList.append(atom)
    bonds.hbonds(interfaceList)
    #pdb=a

def checkOZA():
    global DEBUG
    DEBUG=True
    #pdb=PDBReader.readFile(u"./pdbs/2OZA_FH.pdb",interfaceParts=['B','A'])
    pdb=PDBReader.readFile(u"./pdbs/1UUG_FH.pdb",interfaceParts=['B','A'])
    atom=[a for a in pdb.atoms if a.chain=='A' and a.resId==134 and a.symbol=='ND1'][0]
    around=[a for a in pdb.ktree.findByDistance(query_point=atom.coord, distance=MAX_HBOND) if a!=atom]
    intersting2=[a for a in around if a.chain=='B' and a.resId==61 and a.symbol=='OD1']
    bonds=hbonds(pdb)
    bonds.HDPlusDefinition=False
    bonds.checkHbond(atom,intersting2[0])
    bonds.checkHbond(intersting2[0],atom)

    
def debug():
    pdb=PDBReader.readFile(u"./pdbs/1UUG_FH.pdb",interfaceParts=['B','A'])
    atom=[a for a in pdb.atoms if a.chain=='B' and a.resId==23 and a.symbol=='O'][0]
    around=[a for a in pdb.ktree.findByDistance(query_point=atom.coord, distance=MAX_HBOND) if a!=atom]
    intersting2=[a for a in around if a.chain=='A' and a.resId==63 and a.symbol=='NE2']
    bonds=hbonds(pdb)
    bonds.HDPlusDefinition=False
    bonds.checkHbond(atom,intersting2[0])
    bonds.checkHbond(intersting2[0],atom)
