...
class DonAcceptor:
    ...
    def __probableDonors(self, atom, nextAtom):
        ...
        for donor in self.__donors():
            if atom.symbol==donor['Atom'] and (donor['Residue']==atom.residue or donor['Residue'] == 'Backbone'):
                #old code - using hydrogen dist - the is not good as sometimes the dist is wrong
                #hAtom = nextAtom(atom, donor['Hydrogen (dist)'])
                #if hAtom.symbol==donor['Hydrogen'] and hAtom.resId==atom.resId:
                #    yield (atom, donor['orbital'], hAtom, donor)
                hAtom=nextAtom(atom,1)
                while hAtom.resId==atom.resId:
    ...
    def __probableAcceptors(self, atom, nextAtom):
        for acceptor in  self.__acceptors():
            if atom.symbol==acceptor['Atom'] and (acceptor['Residue']==atom.residue or acceptor['Residue']=='Backbone'):
...
if __name__=='__main__':
    global check
    import pdbReader, hbonds
    oza=pdbReader.PDBReader.readFile('./pdbs/2OZA_FH.pdb')
    hb=hbonds.hbonds(oza)
    check=oza
    oxyASN=[a for a in oza.atoms if a.symbol=='O' and a.residue=='ASN' and a.resId==14]
